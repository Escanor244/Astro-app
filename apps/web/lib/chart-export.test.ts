...
const CHART: Chart = {
  ...
  grahas: GRAHAS.map((name, i) => ({
    ...
    dignity_name: term("Neutral", "சமம்"),
    dignity_reason: "fixture",
    from_exaltation: 90,
    dispositor: 0,
    dispositor_name: term("Sun", "சூரியன்"),
    vargottama: false,
    karakas: [term("Father", "பிதுர் காரகன்")],
    aspects_rasis: [(i + 6) % 12],
    aspects_bhavas: [7],
  })),
  lagna_vargottama: false,
  bhavas: Array.from({ length: 12 }, (_, i) => ({
    number: i + 1,
    name: term("House", "ஸ்தானம்"),
    label: `${i + 1} house`,
    label_ta: `${i + 1}ஆம் பாவகம்`,
    rasi: (1 + i) % 12,
    rasi_name: term("Rasi", "ராசி"),
    signification: "fixture",
    lord: 0,
    lord_name: term("Sun", "சூரியன்"),
    occupants: [],
    groups: [],
    group_names: [],
    aspected_by: [],
  ...
  maraka_lords: [5],
  sevvai: {
    name: term("Sevvai dosham", "செவ்வாய் தோஷம்"),
    mars_rasi: 2,
    mars_rasi_name: term("Gemini", "மிதுனம்"),
    readings: [],
    exemptions: [],
    flagged_conventions: [],
    house_sets: {},
